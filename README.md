...
## Vercel Deployment (avoid `NOT_FOUND`)

If your deployment returns Vercel `NOT_FOUND`, verify these in order:

1. Confirm the URL/path is valid for the deployed environment.
2. Ensure Vercel detects this as a **Next.js** project (this repo now includes `vercel.json` with `framework: "nextjs"`).
3. In Vercel project settings, clear any incorrect custom **Output Directory** so defaults are used.
4. Confirm routes exist in `app/` (for example root route is `app/page.tsx`).

This project is framework-routed Next.js (not a pure SPA), so do **not** add catch-all rewrites to `index.html`.
